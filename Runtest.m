...
%A = gallery('tridiag',k^2,-1,2,-1);
%v = rand(k,1);
%v1 = rand(k,1);
%v2 = rand(k,1);
funcV1 = @(x,y) y.*(y-1).*x.*(x-1);
funcV2 = @(x,y) -2.*(y.*(y-1)+x.*(x-1));
Y = linspace(0,1,k+2)'; X = linspace(0,1,k+2)';
v1 = zeros(k^2,1); v2 = zeros(k^2,1);       % Burde jeg ta med kanten her?
for i = 1:k
   v1(1+(i-1)*k:i*k) = funcV1(X(i+1),Y(2:k+1));
   v2(1+(i-1)*k:i*k) = funcV2(X(i+1),Y(2:k+1));
end
...
%v1 = 1:1:k; v2 = -1:-1:-k; v1 = v1'; v2 = v2';
len_T = 1000; T = linspace(0,2,len_T);
h = T(2)-T(1);
...
[zn1] = intmeth(zn1(end,:),H1,Hn1,len_T,h,M,3);
[zn2] = intmeth(zn2(end,:),H2,Hn2,len_T,h,M,3);
%[zn1] = intmeth(zn1(end,:),H,Hn,len_T,h,M,2);
%[zn1] = intmeth(zn1(end,:),H,Hn,len_T,h,M,3);
%Hn = H(M+1,M);
Hn1 = Hm1; Hn2 = Hm2;
ns = V1(:,1:M)*zn1+V2(:,1:M)*zn2*0;
y = y + ns;
v1 = V1(:,end); v2 = V2(:,end);
diff = max(max(ns));
end
...
solution = @(t,x,y) t*x*(x-1)*y.*(y-1)/(t+1);


% ta med nullene på kanten:
sol = zeros((k+2)^2,len_T);
for i = 1:len_T
    for j = 1:k+2
            sol(1+(j-1)*(k+2):j*(k+2),i) = solution(T(i),X(j),Y);
    end
end
...
ynew = zeros(k+2,k+2,len_T);
ynew(2:k+1,2:k+1,:) =reshape(y,[k,k,len_T]);
y = reshape(ynew,[(k+2)^2,len_T]);



%sol = zeros(k^2,len_T);
%for i = 2:1:len_T %Burde lage en metode som regner ut "korrekt" løsning med differensementoder
    %sol(:,i) = (expm(A*T(i))-eye(k))*(v1*f1(1)+v2*f2(1));
%    sol(:,i) = (expm(A*T(i))-eye(k))*(v1*f1(1));
