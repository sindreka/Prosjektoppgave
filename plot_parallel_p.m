...
avg = 2; k = 40; m = 40; p = [20,40,100,200]; tol = 10^-7; para = 4;

if 1 %compare m, function 1
    j = 1;
    utdata1 = zeros(length(p),3,avg);
    utdata2 = zeros(length(p),3,avg);
    ...
    for i = p
        for a = 1:avg
            utdata1(j,:,a) = parallel( i,k,40,1,1,tol,para);
            utdata2(j,:,a) = parallel( i,k,1 ,3,1,tol,para);
        end
        j = j + 1;
...
if 1 %compare m, function 2
    ...
    for i = p
        for a = 1:avg
            utdata1(j,:,a) = parallel( i,k,40,1,2,tol,para);
            utdata2(j,:,a) = parallel( i,k,1 ,3,2,tol,para);
        end
        j = j + 1;
...
if 1 %compare k, function 2
    ...
    for i = p
        for a = 1:avg
            utdata1(j,:,a) = parallel( m,i,40,1,1,tol,para);
            utdata2(j,:,a) = parallel( m,i,1 ,3,1,tol,para);
        end
        j = j + 1;
    ...
    set(findall(gcf,'-property','FontSize'), 'Fontsize',18)
    set(h,'Location','Best');
    drawnow; print -djpeg c3comp1m
    h = get(0,'children');
    saveas(h(end),'c3comp1k','fig');
end
if 1 %compare k, function 2
    j = 1;
    utdata1 = zeros(length(p),3,avg);
    ...
    for i = p
        for a = 1:avg
            utdata1(j,:,a) = parallel( m,i,40,1,2,tol,para);
            utdata2(j,:,a) = parallel( m,i,1 ,3,2,tol,para);
        end
        j = j + 1;
