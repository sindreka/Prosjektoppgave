...
We now assume that $p$ is non-separable, so that we need to solve $m$ independent problems when using KPM. Several processing units will be used on KPM and KPM$(n)$ when possible, remember that it is not possible to use more than one processing unit on DM. \\% . We will make use of several processing units, together with what we learned in section \ref{sec:seri} to see if we can make KPM outperform DM.
