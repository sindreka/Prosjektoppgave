...
KPM needs to store $A$, $q(t)$, $z(t)$, $V_n$, $H_n$, and an inverted matrix, remember that for \texttt{KPM}; $n = m$. If $p$ is separable we also need to store $v$ and $f(t)$, if $p$ is non-separable we need to store $P(t)$ instead. \\
