...
Each solver was implemented in two versions, one where $p$ was assumed separable, and one where $p$ was assumed to be non-separable. Parallel computations was only implemented for KPM and KPM$(n)$ with $p$ non-separable, because in this case we needed to solve $m$ independent problems, and not just one.\\ %as is the case when $p$ is separable. \\
