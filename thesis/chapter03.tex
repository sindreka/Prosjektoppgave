...
In section \ref{sec:test} some test problems are stared.
...
%\\!!!!!!!!!!!!!!!!!!!!!!!!!!Skriv noe om hvordan $P(t)$ og slikt er lagret!!!!!!!!!!!!!!!!!!\\
%Since computers are discrete, we need to store the functions depending on $t$
%We let $q^l=q(t_l)$, $P^l =P(t_l)$ and $f^l = f(t_l)$ for
...
Each solver was implemented in two versions, one where $p$ was assumed separable, and one where $p$ was assumed to be non-separable. Parallel computations was only implemented for $p$ non-separable, because in this case we needed to solve $m$ independent problems.\\%, and not just one as when $p$ is separable. 
